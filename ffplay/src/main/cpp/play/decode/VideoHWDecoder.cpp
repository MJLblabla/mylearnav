...
void VideoHWDecoder::init() {
    m_PacketQueue = new AVPacketQueue();
    m_PacketQueue->Start();
}

void VideoHWDecoder::unInit() {
...
int VideoHWDecoder::start(AVFormatContext *m_AVFormatContext) {
    // m_StartTimeStamp =-1;
...
void VideoHWDecoder::onDecoderReady() {
...
void VideoHWDecoder::pause() {
    std::unique_lock<std::mutex> lock(m_Mutex);
    m_DecoderState = STATE_PAUSE;
}

void VideoHWDecoder::resume() {
    std::unique_lock<std::mutex> lock(m_Mutex);
    m_DecoderState = STATE_DECODING;
    m_Cond.notify_all();
}
...
void VideoHWDecoder::decodeThreadProc(VideoHWDecoder *videoHWDecoder) {
    videoHWDecoder->dealPackQueue();
}

void VideoHWDecoder::dealPackQueue() {
...
void VideoHWDecoder::UpdateTimeStamp() {
...
void VideoHWDecoder::clearCache() {
