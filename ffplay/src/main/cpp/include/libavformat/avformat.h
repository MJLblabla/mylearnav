...
#define MAX_STD_TIMEBASES (30*12+30+3+6)
    ...
    struct {
        ...
        /**
         * 0  -> play.decoder has not been searched for yet.
         * >0 -> play.decoder found
         * <0 -> play.decoder with codec_id == -found_decoder has not been found
         */
        int found_decoder;
...
#define AVFMT_AVOID_NEG_TS_MAKE_ZERO         2 ///< Shift timestamps so that they start at 0
    ...
    /**
     * Forced video codec.
     * This allows forcing a specific play.decoder, even when there are multiple with
     * the same codec_id.
     * Demuxing: Set by user
     */
    AVCodec *video_codec;
 ...
 * If the play.decoder parameter is non-NULL, av_find_best_stream will find the
 * default play.decoder for the stream's codec; streams for which no play.decoder can
 ...
 * @param decoder_ret       if non-NULL, returns the play.decoder for the
 *                          selected stream
 * @param flags             flags; none are currently defined
 * @return  the non-negative stream number in case of success,
 *          AVERROR_STREAM_NOT_FOUND if no stream with the requested type
 *          could be found,
 *          AVERROR_DECODER_NOT_FOUND if streams were found but no play.decoder
 ...
 * that what is there are valid frames for the play.decoder. It will split what is
 * stored in the file into frames and return one for each call. It will not
 * omit invalid data between valid frames so as to give the play.decoder the maximum
