...
int SoftVideoEncoder::dealOneFrame() {
    ...
    while (!ret) {
        ...
        int64_t pts = av_rescale_q(frame->pts, getTimeBase(), AV_TIME_BASE_Q);
        LOGCATE("EncodeVideoFrame::EncodeVideoFrame  fram  fpts %ld  m_Packet.pts %ld size %d" ,pts,m_Packet->pts,m_Packet->size);
