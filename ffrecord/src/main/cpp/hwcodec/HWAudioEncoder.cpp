...
int HWAudioEncoder::dealOneFrame(MP4Muxer *mMuxer) {
    int result = 0;
    if (m_Exit) {
        return -1;
    }
...
void HWAudioEncoder::flush(MP4Muxer *mMuxer) {
    encodeFrame(nullptr, 0, 0);
    recvFrame(mMuxer);
}

void HWAudioEncoder::recvFrame(MP4Muxer *mMuxer) {
    while (!m_Exit) {
        ...
        if (status == AMEDIACODEC_INFO_TRY_AGAIN_LATER) {
            ...
            AMediaFormat *fmt = AMediaCodec_getOutputFormat(media_codec_);
            const char *s = AMediaFormat_toString(fmt);
            LOGCATE("audio output format %s", s);
            mTrackIndex = mMuxer->addAudioTrack(fmt);
            continue;
        } else {
            if (status < 0) {
                return;
