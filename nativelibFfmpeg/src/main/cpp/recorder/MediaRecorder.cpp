//
// Created by manjialle on 2021/8/19.
//
...
//    NativeImageUtil::CopyNativeImage(inputFrame, pImage);
    m_VideoFrameQueue.Push(inputFrame);
    return 0;
}
...
int MediaRecorder::StopRecord() {
    ...
    if (m_pAudioThread != nullptr || m_pVideoThread != nullptr || m_pMediaThread != nullptr) {
        ...
        if (m_EnableVideo) {
            ...
            av_free(m_VideoStream.pFrameBuffer);
            m_VideoStream.pFrameBuffer = nullptr;
        }
    ...
    while (!vOs->m_EncodeEnd) {
        ...
        if (av_compare_ts(vOs->m_NextPts, vOs->m_pCodecCtx->time_base,
            ...
            LOGCATE("MediaRecorder::StartVideoEncodeThread start queueSize=%d",
                    recorder->m_VideoFrameQueue.Size());
            //视频和音频时间戳对齐，人对于声音比较敏感，防止出现视频声音播放结束画面还没结束的情况
            if (audioTimestamp <= videoTimestamp && aOs->m_EncodeEnd)
                vOs->m_EncodeEnd = aOs->m_EncodeEnd;
...
int MediaRecorder::EncodeAudioFrame(AVOutputStream *ost) {
    ...
    if (frame) {
        ...
        dst_nb_samples = av_rescale_rnd(
                swr_get_delay(ost->m_pSwrCtx, c->sample_rate) + frame->nb_samples,
        ...
        frame->pts = av_rescale_q(ost->m_SamplesCount, (AVRational) {1, c->sample_rate},
                                  c->time_base);
