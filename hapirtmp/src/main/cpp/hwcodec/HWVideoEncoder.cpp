...
int HWVideoEncoder::start(RTMPPush *mRTMPPush, RecorderParam *param) {
...
int HWVideoEncoder::dealOneFrame(RTMPPush *mRTMPPush) {
    int result = 0;
    if (m_Exit) {
        return -1;
    }
...
void HWVideoEncoder::recvFrame(RTMPPush *mRTMPPush) {
    while (!m_Exit) {
...
void HWVideoEncoder::flush(RTMPPush *mRTMPPush) {
